/**
 * @file Master.ino  –  Timer_mit_Uhr
 * @brief Master mit FreeRTOS Tasking (basiert auf Universaltimer v3.2)
 *
 * Erweiterung gegenüber Universaltimer:
 *   - Jedes ESP-NOW-Paket enthält die aktuelle Stunde und Minute
 *   - Der Slave zeigt die Uhrzeit an, solange kein Signal empfangen wird
 *   - Uhrzeit manuell einstellbar per Lang-Druck im Hauptmenü
 */
...
const Theme THEME_FINISH { 0xFFFFAAU, 0x554400U };

TimerData dataToSend;
M5Canvas canvas(&M5.Display);

enum class AppMode { MAIN_MENU, SPORT_TIMER, STOPWATCH, COUNTDOWN, SCOREBOARD, SET_CLOCK };
...
// Uhrzeit-Einstellung: linke Hälfte = Stunden, rechte Hälfte = Minuten
constexpr Zone zClockHourUp  {  0, 25, 160, 85};
constexpr Zone zClockHourDown{  0,155, 160, 65};
constexpr Zone zClockMinUp   {160, 25, 160, 85};
constexpr Zone zClockMinDown {160,155, 160, 65};
constexpr Zone zClockSave    { 40,215, 240, 25};
...
void sendTimerUpdate() {
    ...
    // Aktuelle Uhrzeit in das Paket schreiben
    getCurrentTime(dataToSend.hour, dataToSend.minute);

    // Hauptmenü: Slave soll Uhrzeit anzeigen
    if (currentAppMode == AppMode::MAIN_MENU) {
        dataToSend.state   = STATE_MENU;
        dataToSend.seconds = 0;
        dataToSend.round   = 0;
...
/** Zeichnet den Uhrzeit-Einstellungsdialog. */
void drawSetClock() {
    ...
    M5.Display.drawString("UHR EINSTELLEN", 160, 4);
    ...
    // Pfeile oben
    M5.Display.fillTriangle( 80, 35,  55, 105, 105, 105, 0x00BFFFU);
    M5.Display.fillTriangle(240, 35, 215, 105, 265, 105, 0x00BFFFU);
    ...
    // Doppelpunkt zwischen den Werten
    M5.Display.setTextColor(0x444444U);
    M5.Display.drawString(":", 160, 130);

    // Pfeile unten
    M5.Display.fillTriangle( 80, 215,  55, 155, 105, 155, 0x00BFFFU);
    M5.Display.fillTriangle(240, 215, 215, 155, 265, 155, 0x00BFFFU);

    // Speichern-Button
    M5.Display.fillRoundRect(40, 216, 240, 22, 4, 0x004455U);
    M5.Display.drawRoundRect(40, 216, 240, 22, 4, 0x00BFFFU);
    M5.Display.setFont(&fonts::Font2);
    M5.Display.setTextDatum(middle_center);
    M5.Display.setTextColor(0x00BFFFU);
...
/** Touch-Handling für den Uhrzeit-Einstellungsdialog. */
void handleSetClockTouch(const m5::touch_detail_t& t) {
    ...
    if (t.wasClicked() || rep) {
        if (zClockHourUp.contains(t.x, t.y)) {
            setClockHour = (setClockHour + 1) % 24; changed = true;
        } else if (zClockHourDown.contains(t.x, t.y)) {
            setClockHour = (setClockHour + 23) % 24; changed = true;
        } else if (zClockMinUp.contains(t.x, t.y)) {
            setClockMinute = (setClockMinute + 1) % 60; changed = true;
        } else if (zClockMinDown.contains(t.x, t.y)) {
            setClockMinute = (setClockMinute + 59) % 60; changed = true;
        }
    }

    if (t.wasClicked() && zClockSave.contains(t.x, t.y)) {
        applySetClock();
        // Neue Zeit sofort an Slave senden, damit er nicht die alte weiterführt
        if (sendMutex && xSemaphoreTake(sendMutex, pdMS_TO_TICKS(50)) == pdTRUE) {
            TimerData syncPacket{STATE_PAUSED, 0, 0, setClockHour, setClockMinute};
...
void drawIconReset(int16_t x, int16_t y, uint32_t color) {
    M5.Display.drawArc(x, y, 13, 10, 45, 360, color);
    M5.Display.fillTriangle(x+13, y+8, x+8, y-2, x+18, y-2, color);
}
...
void drawMainMenu() {
    ...
    // Hinweis: Lang drücken für Uhrzeit-Einstellung
    M5.Display.setFont(&fonts::Font0);
    M5.Display.setTextDatum(bottom_center);
    M5.Display.setTextColor(0x333333U);
    M5.Display.drawString("LANG DRUECKEN: UHR STELLEN", 160, 239);

    M5.Display.endWrite();
}

// ==========================================
// Touch-Handling (unverändert)
// ==========================================

void handleMainMenuTouch(const m5::touch_detail_t& t) {
    // Long Press im Hauptmenü → Uhrzeit manuell einstellen
    if (t.wasHold()) {
        uint8_t h, m;
        getCurrentTime(h, m);
        setClockHour   = h;
...
void handleTouch() {
    ...
    if (currentAppMode == AppMode::SET_CLOCK) {
        handleSetClockTouch(t);
        return;
    }
...
void render() {
    if (currentAppMode == AppMode::MAIN_MENU) return;
    if (currentAppMode == AppMode::SET_CLOCK)  return;
    ...
    if (!setupMode && currentAppMode != AppMode::STOPWATCH) {
        int r = 77;
        canvas.drawArc(cx, cy, r, r-3, 0, 360, 0x101010U);
    ...
    if (currentAppMode == AppMode::SPORT_TIMER) {
        ...
        canvas.drawString(buf, cx, cy + 15);
        canvas.setFont(&fonts::Font4);
        canvas.setTextColor(0x888888U);
        canvas.drawString(csBuf, cx, cy + 67);
    } else if (currentAppMode == AppMode::COUNTDOWN) {
...
void timerTask(void* pvParameters) {
    while (true) {
        ...
        // Periodisches Update (alle 1 s) → Slave bleibt synchron, kein Uhrzeit-Fallback
        // Ausnahme: Hauptmenü und Uhrzeit-Einstellung (dort soll Slave die Uhr zeigen)
        static uint32_t lastKeepAlive = 0;
        if (millis() - lastKeepAlive > 500) {
            lastKeepAlive = millis();
            if (currentAppMode != AppMode::SET_CLOCK) {
                sendTimerUpdate(); // auch MAIN_MENU → sendet STATE_MENU an Slave
            }
        }
    }
}
